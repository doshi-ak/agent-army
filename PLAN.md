...
| M6 | **External Automation Lane** *(RATIFIED into §7 by Architect Jul 17 05:16 — spec = `_coordination/Manus-Cowork-Relay-Integration/M6-INTEGRATION-BRIEF.md`, absorbed per Akash's Jul-13 item 4; do NOT re-derive the Manus contract, it is fact-checked in the brief)*: M6.0 webhook intake edge → M6.1 Manus v2 offload adapter (account-level callback, nests under `task_detail` — no per-task `webhook_url`) → M6.2 dispatcher policy (create_task/find_duplicate/dedup_key via M2 state engine) → M6.3 Telegram send-only push (NEVER getUpdates/webhook on the shared bot token — breaks the single-consumer bridge) → M6.4 always-on API-Claude lane (optional, needs API key + public HTTPS host). **§3.8 holds: the SERVER still never calls out — M6 components are session-side/edge adapters, state flows through the M2 tools** | `edge/**` (new, Executor) + PLAN §8 dispatcher policy addendum (Architect) | **BUILD GATED (RULES §1, no session may self-serve):** Akash provides rotated Manus API key + Telegram bot token + webhook endpoint + `/install-github-app`. Acceptance: intake→dedup→offload→callback→push round-trip on a fixture task with zero secrets in tracked files; M6.3 provably send-only; kill-switch documented; Axis-3 scenario added for the lane |
