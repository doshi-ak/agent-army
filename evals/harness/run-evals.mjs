...
const CASES = [
  ...
  },
  {
    ...
    requires: ["skill_forge"], // M3 plugin skill — not a server tool yet
...
for (const c of CASES) {
  ...
  if (missing.length) {
    ...
    else if (missing.includes("skill_forge")) reason = "M3 (plugin skill-forge) not built — not a server tool";
