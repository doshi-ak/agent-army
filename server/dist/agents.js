...
const projectDirArg = z
    .string()
    .optional()
    .describe("Absolute path to the project root. Defaults to the server's working directory (the project that registered this server in .mcp.json).");
/** Render an agent definition file from a role template. */
function renderAgentFile(name, template, assignment) {
    const fmLines = [
        `name: ${name}`,
...
function extractAssignment(body) {
...
function agentFilePath(root, name) {
 ...
 */
function archiveAgentFile(root, name, reason) {
...
Example:
  ...
  - Don't use when: changing an existing agent's role (use agent_assign_role).`,
        ...
        if (fs.existsSync(file)) {
            throw new ToolError(`Agent '${name}' already exists at ${file}. Pick a different name, or use agent_assign_role to re-role it.`);
        }
...
Example:
  - Use when: "retire the docs agent, that work stream is done".`,
        ...
        if (!fs.existsSync(agentFilePath(root, name))) {
            throw new ToolError(`No agent named '${name}' in ${agentsDir(root)}. Use agent_list to see active agents (it may already be archived).`);
        }
...
Example:
  - Use when: "who's on the team right now?"`,
        ...
        const agents = (fs.existsSync(dir) ? fs.readdirSync(dir) : [])
            ...
            const lastActivity = mentions.length > 0
    ...
    server.registerTool("agent_assign_role", {
        title: "Re-role agent",
...
Example:
  ...
  - Don't use when: the agent doesn't exist yet (use agent_create).`,
        ...
        const file = agentFilePath(root, name);
        if (!fs.existsSync(file)) {
            throw new ToolError(`No agent named '${name}' in ${agentsDir(root)}. Use agent_create to staff one, or agent_list to see who exists.`);
        }
        const template = loadRoleTemplate(args.role);
        ...
        const archivedPrevious = archiveAgentFile(root, name, `re-role ${previousRole} -> ${template.role}`);
