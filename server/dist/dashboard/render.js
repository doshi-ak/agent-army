...
import { agentsDir, archiveDir, parseFrontmatter, progressFile, rolesFile, stateFile, teamDir, } from "../shared.js";
...
function teamTable(roster) {
    if (roster.length === 0)
        return "<p><em>No agents staffed yet — run team-new-agent.</em></p>";
    const rows = roster
        .map((m) => `<tr><td>${escapeHtml(m.agent)}</td><td>${escapeHtml(m.role)}</td><td>${statusBadge(m.status)}</td></tr>`)
        .join("\n");
    return `<table><thead><tr><th>Agent</th><th>Role</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table>`;
}
function activeWorkTable(doc, now) {
    if (doc.active_work.length === 0)
        return "<p><em>No active work claimed.</em></p>";
...
function blockersList(doc) {
    if (doc.blockers.length === 0)
        return "<p>None. ✅</p>";
    return `<ul>${doc.blockers.map((b) => `<li>${escapeHtml(b.desc)}</li>`).join("\n")}</ul>`;
}
function progressTail(entries, n) {
    if (entries.length === 0)
        return "<p><em>No progress logged yet.</em></p>";
    const tail = entries.slice(-n).reverse();
    const rows = tail
        .map((e) => `<li><code>${escapeHtml(e.timestamp)}</code> <strong>${escapeHtml(e.actor)}</strong> — ${escapeHtml(e.event)}: ${escapeHtml(e.outcome)}</li>`)
        .join("\n");
...
export function renderDashboardHtml(root, now) {
...
export function refreshDashboard(root, now) {
