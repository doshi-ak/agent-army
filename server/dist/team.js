...
function dashboardSeed(projectName) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
 ...
 */
function skillForgeNudgeText(projectDir) {
    return (`Net-new environment detected (${projectDir} has no .claude/skills/ and no _team/ harness). ` +
...
function ensureMcpJson(root) {
    ...
    if (!fs.existsSync(file)) {
        fs.writeFileSync(file, JSON.stringify({ mcpServers: { "multi-agent-mcp": entry } }, null, 2) + "\n", "utf8");
        return "created";
    }
    let parsed;
    try {
        ...
        throw new ToolError(`${file} exists but is not valid JSON. Fix or remove it, then re-run team_init.`);
    }
    const servers = (parsed.mcpServers ?? {});
    if (servers["multi-agent-mcp"])
        return "skipped"; // already registered; don't clobber user edits
...
function ensureClaudeMdBlock(root) {
...
Example:
  - Use when: "set this project up for an agent team" or after cloning a repo that should run one.`,
        ...
        const netNew = !fs.existsSync(path.join(root, ".claude", "skills")) &&
            !fs.existsSync(teamDir(root));
        const created = [];
        const skipped = [];
        const track = (rel, made) => {
        ...
        return ok(`Team harness ${created.length > 0 ? "initialized" : "already present"} in ${root} (${created.length} created, ${skipped.length} already existed).`, data);
    }));
...
Example:
  - Use when: "where does the team stand?" at session start, or before claiming work.`,
        ...
        if (missing.length === expected.length) {
            throw new ToolError(`No team harness in ${root} (missing ${missing.join(", ")}). Run team_init on this project first.`);
        }
        const agents = (fs.existsSync(agentsDir(root)) ? fs.readdirSync(agentsDir(root)) : [])
            .filter((f) => f.endsWith(".md"))
            .sort()
            .map((f) => {
            const { fm } = parseFrontmatter(fs.readFileSync(path.join(agentsDir(root), f), "utf8"));
            return {
                name: f.replace(/\.md$/, ""),
