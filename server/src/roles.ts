...
export function listRoleNames(): string[] {
  ...
  if (!fs.existsSync(dir)) {
    throw new ToolError(
      `Role catalog directory missing: ${dir}. The server install is incomplete — rebuild the multi-agent-mcp repo.`,
    );
  }
  return fs
    .readdirSync(dir)
...
export function loadRoleTemplate(role: string): RoleTemplate {
  validateName(role, "role");
  const file = path.join(roleTemplatesDir(), `${role}.md`);
  if (!fs.existsSync(file)) {
    throw new ToolError(
      `Unknown role '${role}'. Available roles: ${listRoleNames().join(", ")}. Use role_list for descriptions.`,
    );
  }
...
Example:
  - Use when: inspecting what the 'security-auditor' role actually instructs before staffing one.`,
    ...
    async (args: { role: string }): Promise<ToolResult> =>
      guarded(async () => {
